...
public class WeightedGraph<V> extends UnweightedGraph<V> {
    ...
    private void createWeightedGraph(
        ...
        for (WeightedEdge edge: edges) {
            neighbors.get(edge.u).add(edge); // Additions an edge into the list
        }
    }
    ...
    /** Additions an edge (u, v, weight) to the graph. */
...
//        for (int i = 0; i < cost.length; i++) {
        ...
        while (T.size() < getSize()) {
            ...
            if (u == -1) break; else T.add(u); // Additions a new vertex to T
            totalWeight += cost[u]; // Additions cost[u] to the tree
...
//                isEligible = !T.contains(e.v);
//                isEligible &= cost[e.v] > cost[u] + ((WeightedEdge)e).weight;
//                isEligible &=
                if (!T.contains(e.v)
                        && cost[e.v] > cost[u] + ((WeightedEdge)e).weight) {
                    cost[e.v] = cost[u] + ((WeightedEdge)e).weight;
