 ...
 *
 */
public class OffCutCalculator implements Handleable<CutJob, Cutting>, Runnable {
...
//TODO to avoid the write the method initializeCuttingList I used the following attribute which causes no reasonable solution.
//    private Cutting currentlyOptimized = new Cutting();
     ...
     */
    public OffCutCalculator(DataStream<CutJob> jobQueue, LinkedBlockingQueue<Data<CutJob, Cutting>> handleQueue, Storage storage) {
    ...
    public Cutting handle(CutJob cutJob) {
        ...
        calculateOffcut(order,orderedItems,cutting,cutouts,-1);
        ...
        LOG.info(String.format("For the order %s%s%sThe optimization of the processing process took %dms!%sOptimal solution:%s%s",
                newline, order, newline, duration, newline, newline, cutting));
//TODO update cutouts hier using private methods, which you have to write in this class according to MinVerschnittRechner
    ...
    private void calculateOffcut(Order order, List<Pipe> onOrder, Cutting currentlyOptimized, List<CompoundPipe> proceeds, double rest) {
        if (onOrder.isEmpty()) {
            ...
            if (currentlyOptimized.isBetterCutting(process)) {
                currentlyOptimized = process;
                return;
            }
            if (currentlyOptimized.isPotentiallyBetterCutting(process, rest)) {
